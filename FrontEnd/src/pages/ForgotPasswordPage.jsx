...
import { forgotPassword, clearPasswordMessages } from "../authSlice";
...
const ForgotPasswordPage = () => {
  ...
  useEffect(() => {
    if (forgotPasswordMessage && !error) {
      setTimeout(() => {
        navigate("/reset-password", { state: { email_id: email_id } });
      }, 2000);
    }
  }, [forgotPasswordMessage, error, navigate, email_id]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (email_id && !loading) {
      dispatch(clearPasswordMessages());
      try {
        await dispatch(forgotPassword({ email_id })).unwrap();
      } catch (err) {
